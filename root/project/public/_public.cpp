...
{
  if (m_connfd!=-1) { close(m_connfd); m_connfd=-1; } // if already connected, close it.
  ...
  signal(SIGPIPE,SIG_IGN);  // call signal() to ignore SIGPIPE
  STRCPY(m_ip,sizeof(m_ip),ip); 
  m_port=port;

  struct hostent* h; // struct of ip address
  struct sockaddr_in servaddr; // struct of server address

  if ( (m_connfd = socket(AF_INET,SOCK_STREAM,0) ) < 0) return false; // create socket
...
// buffer: the address of the data receiving buffer, the length of the data is stored in the m_buflen member variable.
// itimeout: the timeout for waiting for data, in seconds, the default value is 0 - wait indefinitely.
// Return value: true - success; false - failure, there are two situations of failure: 1) the wait times out, the value of the m_btimeout member variable is set to true; 2) the socket connection is no longer available.
bool CTcpClient::Read(char *buffer,const int itimeout)
{
  if (m_connfd==-1) return false; // if not connected, return false.

  // the itimeout means, we should wait this time for data to arrive. if no data arrives after itimeout seconds, return false.
  if (itimeout>0)
  {
    struct pollfd fds;
    fds.fd=m_connfd;
...
{
  // if the server socket is >0, close it, there is no special reason for this processing method, don't entangle.
  if (m_listenfd > 0) { close(m_listenfd); m_listenfd=-1; }

  if ( (m_listenfd = socket(AF_INET,SOCK_STREAM,0))<=0) return false;

  // neglect SIGPIPE signal, prevent program from exiting abnormally.
...
// TcpRead: Receive data from the socket connection.
// sockfd: Available socket connection.
// buffer: The address of the data receiving buffer.
// ibuflen: The number of bytes successfully received this time.
// itimeout: The time for receiving timeout, in seconds, -1 - no waiting; 0 - wait indefinitely; >0 - the number of seconds to wait.
// Return value: true - success; false - failure, there are two situations of failure: 1) the wait times out; 2) the socket connection is no longer available.
...
bool CSEM::init(key_t key,unsigned short value,short sem_flg)
{
  ...
  m_sem_flg=sem_flg; //SEM_UNDO is a flag that can be used with semaphores to specify that semaphore operations should be automatically undone when a process finishes.
  ...
  // 获取信号量。
  if ( (m_semid=semget(key,1,0666)) == -1) // key is kind of id of semaphore,1 semephore like mutex, o666
  // fails return -1
  {
    // ENOENT is set when no semaphore set exists for key and IPC_CREAT was not specified in sem_flags
    if (errno==2)
    {
...
bool CSEM::P(short sem_op)
{
  ...
  if (semop(m_semid,&sem_b,1) == -1) { perror("p semop()"); return false; } // semop() is a system call and lock
...
{
  if (m_pos!=-1) return true; //  already added into shared memory return true

  if (m_sem.init(SEMKEYP) == false)  // intialize the sem fails
  {
    if (logfile!=0) logfile->Write("fail to create semaphore (%x)。\n",SEMKEYP); 
    else printf("fail to create semaphore(%x)。\n",SEMKEYP);
  ...
  // 将共享内存连接到当前进程的地址空间。
  m_shm=(struct st_procinfo *)shmat(m_shmid, 0, 0); // attach to the memory
  ...
  // 心跳时since it already die, ，会向进程id发送退出信号，这个信号将误杀当前进程。
