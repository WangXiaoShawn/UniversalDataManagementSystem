...
{
  connection conn;   // 数据库连接类。//create the connection object

  // 登录数据库，返回值：0-成功；其它是失败，存放了MySQL的错误代码。
  // 失败代码在conn.m_cda.rc中，失败描述在conn.m_cda.message中。
  //if (conn.connecttodb("127.0.0.1,root,mysqlpwd,mysql,3306","utf8")!=0)
  // ip, username,password,dbname,port
  if(conn.connecttodb("127.0.0.1,root,cis623,mysql,3306","utf8")!=0) // equal to 0  means success
  {
