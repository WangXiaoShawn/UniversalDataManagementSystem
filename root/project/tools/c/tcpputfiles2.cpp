...
void _help()
{
  ...
  printf("Sample: ./tcpputfiles ~/root/log/idc/tcpputfiles_surfdata.log \"<ip>10.0.2.15</ip><port>5005</port><ptype>1</ptype><clientpath>/home/cis623-vm/root/tmp/tcp/surfdata1/</clientpath><clientpathbak>~/root/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>~/root/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata2</pname>\"\n");
...
bool _tcpputfiles()
{
  ...
  while (true)
  {
    memset(strsendbuffer,0,sizeof(strsendbuffer));
    memset(strrecvbuffer,0,sizeof(strrecvbuffer));
    printf("reach here2.1.\n");
    // 遍历目录中的每个文件，调用ReadDir()获取一个文件名。
    int i=Dir.ReadDir();
    //printf("the ReadDir: %s",(char*)i);
    if (Dir.ReadDir()==false) break; // 
    printf("%s",Dir.m_FullFileName);
